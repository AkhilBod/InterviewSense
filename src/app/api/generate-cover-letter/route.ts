...
export async function POST(req: Request) {
    ...
    try {
        ...
        let promptText = `You are an expert cover letter writer. Your primary goal is to generate a COMPLETE and COMPELLING cover letter.
The letter will use a few specific structural placeholders (like \`[Your Name]\`, \`[Date]\`) ONLY for candidate/company contact information, date, and salutation. These structural placeholders will be programmatically filled in later by our system.

**CRITICAL INSTRUCTION FOR THE LETTER BODY (ALL PARAGRAPHS):**
The BODY of the cover letter (introduction, paragraphs about skills, experience, fit for the role, motivation, conclusion) MUST be FULLY WRITTEN BY YOU. It must be a complete narrative.
**ABSOLUTELY NO VISIBLE BRACKETED PLACEHOLDERS (e.g., "[insert skill here]") SHOULD REMAIN IN THE FINAL BODY TEXT YOU PRODUCE. SHOULD HAVE PROPER SPACES AND BE IN AN EMAIL FORMAT.**

**HOW TO WRITE A COMPLETE AND ENGAGING LETTER BODY AND**

1.  **PRIORITIZE AND INTEGRATE PROVIDED INFORMATION:**
    * **Resume Data (If Provided):** You will receive the candidate's resume content (it will be attached as a file). You MUST thoroughly analyze it. Extract specific skills (e.g., "Python," "Java," "Agile methodologies"), experiences (roles like "Software Engineer at ABC Corp," specific projects, accomplishments like "increased user engagement by 15%"). You MUST then weave these *exact, extracted details* directly into complete, natural-sounding sentences within the letter body.
        * **Correct Example:** If resume lists "Python, C++," your body text should include something like: "My technical expertise includes Python and C++."
        * **Incorrect Example (DO NOT DO THIS):** "[mention specific programming languages from resume - e.g., Python, Java, C++]"
    * **Job Description (If Provided):** Analyze the job description for key requirements, responsibilities, and company values. Tailor the letter body to specifically address how the candidate's skills and experiences (from their resume, or plausibly generated by you if the resume is sparse) align with these points.

2.  **GENERATING RELEVANT CONTENT FOR INFORMATION GAPS (THIS IS KEY):**
